// src/screens/EditCategoryScreen.js - Pantalla de edición de categorías
...
const EditCategoryScreen = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    parentId: initialCategory?.parentId || initialCategory?.parent || null,
    order: initialCategory?.order || 0,
    isActive: initialCategory?.isActive ?? true,
    showOnHomepage: initialCategory?.showOnHomepage ?? false
  });
  const [image, setImage] = useState(null);
  const [imagePreview, setImagePreview] = useState(initialCategory?.image || null);
  ...
  useEffect(() => {
    if (initialCategory) {
      ...
      setImagePreview(initialCategory.image || null);
    }
  }, [initialCategory]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateCategory(initialCategory._id || initialCategory.id, categoryData);
  ...
  return (
    <AuthenticatedWrapper title="Editar Categoría" subtitle={formData.name}>
      <KeyboardAvoidingView
            ...
            <View style={styles.noteCard}>
              ...
              <View style={styles.noteContent}>
                ...
                <Text style={styles.noteText}>
                  Los cambios en las categorías afectan la organización de los productos. 
                  Ten cuidado al modificar la estructura jerárquica.
                </Text>
              </View>
            </View>
          </View>
